...
# apply normalization and create training and test sets
tennis3_n <- as.data.frame(lapply(tennis3[2:83], normalize))
tennis3_n$rowid <- tennis3$rowid
tennis3_n$year <- tennis3$year
tennis3_n$player_won_ind <- tennis3$player_won_ind
tennis3_train_n <- tennis3_n[tennis3_n$year=='2014'|tennis3_n$year=='2015',]
tennis3_test_n <- tennis3_n[tennis3_n$year=='2016',]

tennis3_train_n <- tennis3_train_n[-84] #remove year
tennis3_train_n <- tennis3_train_n[-83] #remove rowid
tennis3_test_n <- tennis3_test_n[-84] #remove year
tennis3_test_n <- tennis3_test_n[-83] #remove rowid
```
...
|-------------------------------------|	:------------------------------------:|	:------------------------------------:|	:------------------------------------:|	:------------------------------------:|
|normalized dataset - all variables   |	1                                    |	0.692064                             |	0.6685785                            |	1 min                                |
|normalized dataset - all variables   |	2                                    |	0.717687                             |	0.6526014                            |	8 min                                |
...
|normalized dataset - logreg variables|	1                                    |	0.6907029                            |	0.6800492                            |	30 sec                               |
|normalized dataset - logreg variables|	2                                    |	0.6907029                            |	0.6837362                            |	5.5 min                              |
|normalized dataset - logreg variables|	3                                    |	0.697506                            |	0.6763621                            |	8 min                                 |
|normalized dataset - logreg variables|	4                                    |	0.6979592                            |	0.6636624                            |	10 min                               |
...
matchup_avg_bpSaved_allowed_perSet_last20Surface_diff+
matchup_perc_bpSaved_allowed_last20Surface_diff
, data = tennis3_train_n, hidden=2, lifesign='full', lifesign.step=10000, threshold=.01, stepmax = 100000000)  
...
### Prediction Using Ensemble/stacked Model, Evaluating & Comparing the Models - CHARLIE TO COMPLETE
...
# try ANN models with varying numbers of nodes and 1 layer
acc <- 0
best_nodes <- 0
...
{
  print(i)
  ann_model <- neuralnet(formula = player_won_ind ~ .,
                              data = tennis3_train_n, hidden=i)  
  ann_predictions <- neuralnet::compute(ann_model, tennis3_test_n[1:82])
  ann_predictions <- as.factor(ifelse(ann_predictions$net.result[,1]>ann_predictions$net.result[,2],0,1))
  ann_model_confmatrix <- confusionMatrix(ann_predictions, tennis3_test_n$player_won_ind)
  print(ann_model_confmatrix$overall['Accuracy'])
}


# try ANN models with varying numbers of nodes and 2 layers
layer1_nodes <- 0
layer2_nodes <- 0
for (i in 1:2)
{
  ...
  {
   ann_model <- neuralnet(formula = player_won_ind ~ .,
                              data = tennis3_train, hidden=c(i,j))  
  ann_predictions <- neuralnet::compute(ann_model, tennis3_test[1:82])
  ann_predictions <- as.factor(ifelse(ann_predictions$net.result[,1]>ann_predictions$net.result[,2],0,1))
  ann_model_confmatrix <- confusionMatrix(ann_predictions, tennis3_test$player_won_ind)
  if (ann_model_confmatrix$overall['Accuracy'] > acc)
  {
    acc <- ann_model_confmatrix$overall['Accuracy']
    layer1_nodes <- i
    layer2_nodes <- j
  }
  }
...
#choose the number of nodes with the highest accuracy
ann_model <- neuralnet(formula = player_won_ind ~ .,
                              data = tennis3_train, hidden=c(3,))
...
### Prediction Using Decision Tree Model - AMRIT TO COMPLETE
```{R, echo=True, eval=TRUE}
#Below we are brinning in the library (C50) and, again, looking at data with a decision tree lense.
library(C50)
str(tennis3)

#Below we create the first decision tree model with all the data from the cleaned up dataset and plot.This step was done to just see how complex the tree will be and get a basic understanding.
tennismod1 <- C5.0(player_won_ind ~., data = tennis3)
plot(tennismod1)
table(tennis$player_won_ind)

#Since already split the data into test and train at the very beginnin, we are now checking the proportion of class variables - meaning are they similar when it comes to percent chance of winning or losing. We can see that both test and train data are very similar with roughly 50% wins and losses.
prop.table(table(tennis3_train$player_won_ind))
prop.table(table(tennis3_test$player_won_ind))

#Below we are creating the simplest decision tree using the training data we created in the beginning
library(C50)
tennismatch_DTmodel1 <- C5.0(tennis3_train[-83], tennis3_train$player_won_ind)

#Here we looked at the summary of the model to see what decision point seems to be the main factor. We noted that matchup_rank_diff is the first important factor and whether the player's difference in rank was less than or greater than 3. Additionally, we noted there are 28 nodes in this decision tree and on the training data it managed to predict it with an error rate of 31.1%; thus, an accuracy rate of 69.7%.Lasty, we noted the top 2 relative variances of the variables in the model were: 1. matchup_rank_diff and 2. matchup_avg_rankPoints_lastSeason_diff..
summary(tennismatch_DTmodel1)

#To evaluate the model performance, we created a factor vector of predictions on test data
dtmodel1_match_pred <- predict(tennismatch_DTmodel1, tennis3_test)

#To get a confusion matrix, to understand this base model, we created a cross tabulation of predicted versus actual classes. From this model we can see, that the test data had a false positive prediction rate of 19.1% and a false negative rate of 14%. What is worse when gambling, in our opinion, is predicting a win when there is actually a loss (a false positive) as the player loses money on bets he placed with confidence.
library(gmodels)
CrossTable(tennis3_test$player_won_ind, dtmodel1_match_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual win', 'predicted win'))

#Based on the above, we see that there is room to improve our decision tree model. We will make false poistives more costly in our model by creating an error matrix. This is to put more weight on false positive errors and to have the model correct those. 
dt1_error_cost <- matrix(c(0, 4, 1, 0), nrow = 2)
dt1_error_cost

#Below we apply the error cost matrix to our decision tree model (calling model now with the error_cost "DTmodel2") and see the results. As we expected are false positives did go down, but at the cost of losing opportunities of winning, as we saw our false negative percentage increase dramatically.
tennismatch_DTmodel2 <- C5.0(tennis3_train[-83], tennis3_train$player_won_ind,
                          costs = dt1_error_cost)
dtmodel2_match_pred <- predict(tennismatch_DTmodel2, tennis3_test)
CrossTable(tennis3_test$player_won_ind, dtmodel2_match_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual win', 'predicted win'))



#Thus, the team wanted to see what the results would be after boosting the training data with more randomness (50 trials). We can see that there was very little improvement from the original decision tree model with false positive percentages going from 19.1% to 13.4% at the cost of false negatives increasing.
tennismatch_DTmodel3_boost50 <- C5.0(tennis3_train[-83], tennis3_train$player_won_ind,
                       trials = 50)
tennismatch_DTmodel3_boost50
summary(tennismatch_DTmodel3_boost50)

dtmodel3_boost_pred50 <- predict(tennismatch_DTmodel3_boost50, tennis3_test)
CrossTable(tennis3_test$player_won_ind, dtmodel3_boost_pred50,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual default', 'predicted default'))

# Lastly, the team tried to combine both boosting and cost matrix into a new decision tree model, to see how that results would look. Oddly, combining the two, significantly decreased false negatives but our false positive rate went up to 25.9%.
tennismatch_DTmodel4 <- C5.0(tennis3_train[-83], tennis3_train$player_won_ind, costs = dt1_error_cost,
                       trials = 50)


tennismatch_DTmodel4
summary(tennismatch_DTmodel4)

dtmodel4_pred <- predict(tennismatch_DTmodel4, tennis3_test)
CrossTable(tennis3_test$player_won_ind, dtmodel4_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual default', 'predicted default'))
...
### Prediction Using Random Forest Model - AMRIT TO COMPLETE
```{R, echo=True, eval=TRUE}
#Bring in library (randomForest)
library(randomForest)

#Below we built a basic randomforest model to see what the estimated error rate is but to also see how many variables were looked at per tree. As predicted, since we have roughly 83 columns, there are 9 varaibles tried at each tree.
tennismatch_RFmodel1 <-randomForest(player_won_ind ~ ., data = tennis3_train)
tennismatch_RFmodel1

#To evaluate the model performance, we created a factor vector of predictions on test data similar to our decision tree model.
rfmodel1_match_pred <- predict(tennismatch_RFmodel1, tennis3_test)

#To get a confusion matrix, to understand this random forest base model, we created a cross tabulation of predicted versus actual classes like we did with decision trees.
CrossTable(tennis3_test$player_won_ind, rfmodel1_match_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual win', 'predicted win'))

#Based on the above, we see that there is room to improve our decision tree model. We will make false poistives more costly in our model by creating an error matrix. This is to put more weight on false positive errors and to have the model correct those. 
rf1_error_cost <- matrix(c(0, 4, 1, 0), nrow = 2)
rf1_error_cost

#Below we apply the error cost matrix to our decision tree model (calling model now with the error_cost "RFmodel2") and see the results. From the results, it did not seem like error cost matrix did anything in terms of changing these results.
tennismatch_RFmodel2 <- randomForest(player_won_ind ~ ., data = tennis3_train,
                          costs = rf1_error_cost)
rfmodel2_match_pred <- predict(tennismatch_RFmodel2, tennis3_test)
CrossTable(tennis3_test$player_won_ind, rfmodel2_match_pred,
           prop.chisq = FALSE, prop.c = FALSE, prop.r = FALSE,
           dnn = c('actual win', 'predicted win'))
...
>>>>>>> 13c6c9cdeb188f0f656ab634fe66e44f3dc725ed
