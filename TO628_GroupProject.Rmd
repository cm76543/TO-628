...
# ANN with 1 hidden layer and 
ann_model <- neuralnet(formula = player_won_ind ~ .,
                              data = tennis3_train, hidden=3)
...
ann_predictions <- compute(ann_model, pokemon_test_n[1:71])
ann_predictions <- as.factor(ifelse(ann_predictions$net.result[,1]>ann_predictions$net.result[,2],0,1))
ann_model_confmatrix <- confusionMatrix(ann_predictions, pokemon_test_n$is_legendary)
